...
### Maintaining the training workload

The profile is generated by `scripts/pgo-build.sh --profile-only` using:

- Pre-generated SQL dumps from `scripts/generate-training-sql.py`
- The OLTP / OLAP / late-materialisation benchmark suite under `bench/`
...
1. If `scripts/pgo-build.sh` exits with "no training SQL dumps found", run
   `python3 scripts/generate-training-sql.py --all` to regenerate them.
2. If a benchmark script signature changed (new flags, renamed arguments),
   update the corresponding `python3 bench/*.py` invocation in the script.
3. If `llvm-profdata` is not found, install the LLVM tools:
   `rustup component add llvm-tools-preview`.
4. If the profile format changes across Rust toolchain upgrades, delete
   `target/pgo/` and re-run the pipeline.

### Verifying PGO locally
