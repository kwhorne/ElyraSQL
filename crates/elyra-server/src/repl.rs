...
mod tests {
    ...
    /// Handshake helpers read process-global env vars; serialize the tests
    /// that touch them so parallel test threads cannot observe each other's
    /// mutations. Async mutex because the guard is held across `.await`.
