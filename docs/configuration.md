...
| `ELYRASQL_QUERY_TIMEOUT_MS` | `0` (off) | Per-query wall-clock timeout: a statement running longer returns `query exceeded ELYRASQL_QUERY_TIMEOUT_MS`. **It only interrupts a statement that yields** (waiting on I/O, or between async steps). It cannot preempt a long *synchronous* CPU loop — that work runs to completion and the client stays blocked until it does — so this is not a CPU-DoS guard (see [limitations](limitations.md)). Tracked as ESQL-32. |
| `ELYRASQL_MAX_STRING_BYTES` | `67108864` (64 MiB) | Byte budget for one string-expanding result (`REPEAT`, `SPACE`, `LPAD`, `RPAD`). A larger result yields `NULL` instead of allocating unbounded memory — the same behaviour as MySQL past `max_allowed_packet` (whose default is also 64 MiB). |
