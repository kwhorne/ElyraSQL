...
| `ELYRASQL_CLUSTER_TLS_CERT` / `ELYRASQL_CLUSTER_TLS_KEY` | unset | PEM certificate + key for the **primary's** replication endpoint. When both are set the replication transport is TLS-encrypted; unset = plaintext (warned). |
| `ELYRASQL_CLUSTER_TLS_CA` | unset | PEM root(s) the **replica** uses to *verify* the primary's replication certificate (for a self-signed cert, point it at that cert). Set = connect over TLS with verification; a mismatched cert is rejected. |
| `ELYRASQL_CLUSTER_TLS_SERVER_NAME` | `localhost` | Server name the replica expects in the primary's certificate (must match its SAN). |
