...
> **Stable release: v1.4.10.** A broad, MySQL-compatible SQL engine: full
...
> (`SHOW` + `INFORMATION_SCHEMA`), vector + full-text + hybrid + faceted search, and
...
SELECT VERSION();   -- 8.0.0-ElyraSQL-1.4.10
```
...
curl -L -o elyrasql.tar.gz \
  https://github.com/kwhorne/ElyraSQL/releases/download/v1.4.10/elyrasql-1.4.10-linux-x86_64.tar.gz
tar xzf elyrasql.tar.gz && ./elyrasql-1.4.10-linux-x86_64/elyrasql serve
```
...
```bash
docker run -p 3307:3307 -v elyra:/var/lib/elyrasql ghcr.io/kwhorne/elyrasql:1.4.10
