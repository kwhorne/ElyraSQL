...
#
# Modes:
#   ./scripts/pgo-build.sh              # full pipeline -> target/release/elyrasql
...
BENCH_BIN="$ROOT/target/release/elyrasql"
...
RUSTFLAGS="-Cprofile-generate=$PROFRAW_DIR" cargo build --release --locked -p elyra-cli
...
if [[ "$PROFILE_ONLY" == "1" ]]; then
  ...
  echo "    RUSTFLAGS=\"-Cprofile-use=$PROFDATA -Cllvm-args=-pgo-warn-missing-function\" cargo build --release --locked -p elyra-cli"
  exit 0
fi

echo "==> [4/4] optimized build (profile-use)"
RUSTFLAGS="-Cprofile-use=$PROFDATA -Cllvm-args=-pgo-warn-missing-function" \
  cargo build --release --locked -p elyra-cli
